...
function computeFeedRelevance(item: IntelligenceFeedItem, selectedCountry: string | null): number {
  ...
  const recencyScore = ageHours <= 1 ? 1 : ageHours <= 6 ? 0.88 : ageHours <= 24 ? 0.72 : ageHours <= 72 ? 0.55 : 0.35;
  const countryBoost = selectedCountry && item.country === selectedCountry ? 0.2 : 0;
  ...
  return backendScore * 0.7 + recencyScore * 0.2 + riskScore * 0.1 + countryBoost;
}
  ...
  const fetchFeed = useCallback(async () => {
    try {
      const data = await getGlobalIntelligenceFeed({
        ...
        limit: Math.max(maxRows * 8, selectedCountry ? 24 : 50),
      });
